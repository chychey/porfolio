...
def load_gui(self):
    ...
    self.lbl_1name.grid(row=2,column=0,padx=(27,0),pady=(10,0),sticky=N+W)
    ...
    self.btn_add = tk.Button(self.mmaster,width=12,height=2,text='Add',command=lambda: phonebook_func.addToList(self))
    self.btn_add.grid(row=5,column=0,padx=(25,0),pady=(45,10),sticky=W)
    self.btn_update = tk.Button(self.mmaster,width=12,height=2,text='Update',command=lambda: phonebook_func.onUpdate(self))
    self.btn_update.grid(row=8,column=1,padx=(15,0),pady=(45,10),sticky=W)
    self.btn_delete = tk.Button(self.mmaster,width=12,height=2,text='Delete',command=lambda: phonebook_func.onUpdate(self))
    self.btn_delete.grid(row=8,column=2,padx=(15,0),pady=(45,10),sticky=W)
    self.btn_close = tk.Button(self.mmaster,width=12,height=2,text='Close',command=lambda: phonebook_func.onUpdate(self))
